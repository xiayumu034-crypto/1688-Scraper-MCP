...
### 3. 供应商深度防坑背调 (`analyze_supplier_reliability` & `get_product_reviews`)
买货最怕被坑、发货慢、品控差。这个工具专门提取 1688 右侧边栏的店铺核心履约指标 (BSR) 和买家真实评价：
- **工厂资质**：牛头标超级工厂、实力商家。
- **履约数据**：发货速度、响应速度、货描相符评分。
- **深度评论翻页**：工具能自动模拟人手点击“下一页”按钮，支持突破首屏限制，拉取 100 条甚至上千条真实评价（通过 `max_count` 参数控制）。
- **AI 赋能玩法**：发两个链接给 AI：“帮我对比一下这两个厂家，谁的退换货率更低？再帮我把链接A的所有评价抓下来，总结一下买家反馈的色差问题。”
